...
public abstract class SellInvoicePageBase : QueryPersistedReportPage<SellInvoiceFilterVm>
{
    protected List<SellInvoiceRpResponse>? Data;
    ...
    [
        ...
        new("فاکتور فروش", ClientRoutes.Reporting.SellInvoices, icon: Icons.Material.Filled.PointOfSale)
    ];
    ...
    protected async Task OnSubmit(ReportFilterVmBase filter)
    {
        ...
        await LoadReportAsync((SellInvoiceFilterVm)filter);
    }

    protected async Task LoadReportAsync(SellInvoiceFilterVm model)
    {
        var request = model.ToRequest();

        IsLoading = true;

        Data = await SendRequestAsync<IReportingService, List<SellInvoiceRpResponse>>(
